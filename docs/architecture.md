...
| `ai/commands/` | Agent command definitions (e.g., what happens when a user types `/add_project`). |
...
- Agents can **only write** inside `projects/<active-project>/`.
- Agents must **never modify** `ai/`, `.cursor/`, `.claude/`, or `.antigravity/`.
- Scripts **never run** `rm -rf` on symlink targets — only the symlink is removed.
- `.claude/settings.json` explicitly denies `rm -rf`, `rm -r`, and access to `.env` and `~/.ssh/`.
- The `.gitignore` excludes `projects/` (contains user-specific symlinks), `WORKSPACE.md` (dynamic state), `repos/_registry.yaml` (contains local paths), and `.claude/projects-context.md` (auto-generated).
