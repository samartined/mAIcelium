...
| `sync_symlinks.sh` | Rebuild all symlinks — cleans broken ones, recreates from `ai/` and active projects | `bin/sync_symlinks.sh` |
...
Run this after:
- Adding new files to `ai/rules/` or `ai/skills/`
- Moving the workspace to a different path
- Recovering from broken symlinks
- Any manual changes to the `ai/` directory

---

## Agent commands

These commands can be used in IDEs that support agent commands (e.g., Claude Code's `/command` syntax). Commands with fuzzy matching use Python scripts in `ai/commands/scripts/`.

| Command | Description | Fuzzy | Defined in |
|---------|-------------|-------|------------|
| `/add_project <name>` | Fuzzy-match a project from the registry and plug it in | ✔ | `ai/commands/add_project.md` |
| `/remove_project <name>` | Fuzzy-match a linked project and unplug it | ✔ | `ai/commands/remove_project.md` |
| `/list_projects` | Show all currently linked projects | — | `ai/commands/list_projects.md` |
| `/workspace_status` | Show active projects, skills, rules, and symlink status | — | `ai/commands/workspace_status.md` |
| `/git_backup [message]` | Stage, commit, and push workspace changes | — | `ai/commands/git_backup.md` |

For Claude Code, these commands are wired through `.claude/commands/*.yaml` files that point to the definitions in `ai/commands/`.

### Fuzzy matching

The `/add_project` and `/remove_project` commands support fuzzy matching via Python scripts (`ai/commands/scripts/`). The matching resolves approximate names in this order:
...
All rules live in `ai/rules/`. They are prescriptive — agents must follow them.
...
| **Workspace Conventions** | `workspace-conventions.md` | `ai/` as source of truth, naming conventions (kebab-case), command output format, file placement reference |
...
Skills live in `ai/skills/` organized by category. Each skill has a `SKILL.md` with instructions.
...
| **Workspace Guide** | `_common/workspace-guide/` | Auto-orientation for agents: always write to `ai/`, naming conventions, command output format, symlink architecture. |
...
Reusable prompt templates live in `ai/prompts/`. They use `{{PLACEHOLDER}}` syntax for variable substitution.
...
|------|---------|------------|
| `CLAUDE.md` | Entry point for Claude Code agents — points to `ai/`, `WORKSPACE.md`, and `.claude/projects-context.md` | Manual (version-controlled) |
...
| `.claude/settings.json` | Claude Code permissions (allowed/denied bash commands) | Generated by `init.sh`, can be edited |
...
|-----|----------------|---------------------|-----------------|
| **Cursor** | `.cursor/rules/`, `.cursor/skills-cursor/` | Per-file symlinks from `ai/` | `<project>--<rule>` prefixed symlinks |
| **Claude Code** | `CLAUDE.md`, `.claude/` | Direct file access, reads `ai/` paths from `CLAUDE.md` | `.claude/projects-context.md` (auto-generated) |
| **Antigravity** | `.antigravity/rules`, `.antigravity/skills` | Directory-level symlinks to `ai/` | Via directory symlinks (sees everything in `ai/`) |
