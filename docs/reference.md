...
|--------|---------|-------|
| `init.py` | Initialize a fresh workspace — creates directories, symlinks, and config files | `bin/init.py` |
| `add_project.py` | Plug in a project by creating symlinks and importing its rules/skills | `bin/add_project.py <name> <path>` |
| `remove_project.py` | Unplug a project — removes symlinks, original repo untouched | `bin/remove_project.py <name>` |
| `sync_symlinks.py` | Rebuild all symlinks — cleans broken ones, recreates from `mesh/` and active projects | `bin/sync_symlinks.py` |
| `separate_git.py` | Move `.git` outside workspace to avoid IDE conflicts with linked projects | `bin/separate_git.py` |
...
| `_lib.py` | Shared functions (sourced by other scripts, not run directly) | — |
...
| `.claude/settings.json` | Claude Code permissions (allowed bash commands, PreToolUse hooks for write/bash protection) | Generated by `init.py`, can be edited |
